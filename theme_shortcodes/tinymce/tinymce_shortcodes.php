...
class ThemeBlvd_TinyMCE_Shortcodes {
	...
	function init() {
		...
		if ( ( current_user_can( 'edit_posts' ) || current_user_can( 'edit_pages' ) ) && get_user_option('rich_editing') == 'true' )  {
		  	...
		  	//ThemeBlvd Framework URL
		  	$themeblvd_framework_url = get_template_directory_uri().'/framework';
		  	$plugin_url = plugins_url().'/contentplugin/theme_shortcodes/tinymce';
		  	
		  	//Pass URL to plugin's js file
		  	$name = 'BALLS';
   			wp_localize_script( 'ThemeBlvdShortcode', 'custom', array('name' => $name));
			...
			wp_register_style( 'themeblvd-tinymce-shortcodes', $plugin_url.'/layout/css/tinymce_shortcodes.css' );
	...
	function filter_mce_external_plugins( $plugins ) {
        ...
        $plugins['ThemeBlvdShortcodes'] = plugins_url().'/contentplugin/theme_shortcodes/tinymce/editor_plugin.php';
